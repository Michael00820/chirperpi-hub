...
export interface Group {
  ...
  createdAt: string;
  updatedAt: string;
  minPiBalance?: number; // For Pi-gated groups
}
...
export interface Proposal {
  ...
  totalVotes: number;
  votes: Vote[];
  executionData?: any; // For storing execution parameters
  executedAt?: string;
  createdAt: string;
  updatedAt: string;
}
...
export interface Vote {
  ...
  optionId: string;
  voteWeight: number; // Based on Pi balance or verification level
  signature: string; // Pi wallet signature
  votedAt: string;
}
...
export interface CastVoteRequest {
  proposalId: string;
  optionId: string;
  signature: string; // Pi wallet signature for verification
}
...
export interface Message {
  ...
  piTransaction?: PiTransactionData; // For Pi transaction messages
  piPaymentRequest?: PiPaymentRequestData; // For payment requests
  replyToMessageId?: string; // For replies
  replyToMessage?: Message; // For replies
...
export interface MessageReaction {
  ...
  emoji: string; // Pi-themed emojis like ❤️, 💚, 🚀, etc.
  createdAt: string;
}
...
export interface ServerToClientEvents {
  ...
  messageRead: (data: { messageId: string; userId: string; conversationId: string }) => void;
  conversationUpdated: (conversation: Conversation) => void;
  conversationLoaded: (conversation: Conversation) => void;
  userJoined: (data: { conversationId: string; userId: string; timestamp: string }) => void;
  userLeft: (data: { conversationId: string; userId: string; timestamp: string }) => void;
}
